...
function Home() {
  ...
  const handleSubmit = async (event) => {
    ...
    if (!isSlotAvailable(dayBookings, form.time, Number(form.duration), Number(form.courts))) {
    ...
    try {
      ...
      setSlotError(
        err.status === 409 || /courts available/i.test(err.message)
          ? 'Sorry, not enough courts available for this date and time. Please pick another slot or fewer courts.'
                ...
                <form className="pp-form" onSubmit={handleSubmit}>
                  ...
                  <label className="pp-field">
                    ...
                    <select name="time" value={form.time} onChange={handleChange} required>
                      ...
                      {timeSlots.map((slot) => {
                        const count = form.date ? countBooked(dayBookings, slot.value) : 0
